...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // *** تغییر کلیدی در اینجا اتفاق می‌افتد ***
      if (serverResponse.startsWith("LOGIN_SUCCESS")) {
        // پاسخ جدید را با "::" تفکیک می‌کنیم
        final parts = serverResponse.split('::');

        // parts[0] = "LOGIN_SUCCESS"
        // parts[1] = username
        // parts[2] = email
        // parts[3] = credit
        // parts[4] = subscription_tier
        // parts[5] = subscription_expiry

        if (parts.length >= 6) {
          // اطلاعات دریافت شده از سرور را به UserAuthProvider می‌دهیم
          await Provider.of<UserAuthProvider>(context, listen: false)
              .loginWithData(
            username: parts[1],
            email: parts[2],
            token: "simulated-token-for-${parts[1]}", // توکن همچنان نمونه است
            credit: double.tryParse(parts[3]) ?? 0.0,
            tierString: parts[4],
            expiryString: parts[5],
          );

          if (mounted) {
            Navigator.of(context).pop(); // بازگشت به صفحه پروفایل
          }
        } else {
          // اگر فرمت پاسخ سرور اشتباه بود
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Login Failed: Invalid server response format.')),
            );
          }
        }
      } else {
        // اگر سرور خطا برگرداند (مثل USER_NOT_FOUND)
        final errorMessage =
