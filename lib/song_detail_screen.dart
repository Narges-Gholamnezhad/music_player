...
class _SongDetailScreenState extends State<SongDetailScreen> {
  ...
  Stream<_PositionData> get _positionDataStream =>
      Stream.periodic(const Duration(milliseconds: 200), (_) {
        ...
        return _PositionData(
          ...
          prev.bufferedPosition.inMilliseconds ==
              next.bufferedPosition.inMilliseconds);
  ...
  void initState() {
    ...
    if (currentGlobalNowPlaying != null &&
        currentGlobalNowPlaying.song.uniqueIdentifier ==
            widget.initialSong.uniqueIdentifier) {
      if (_arePlaylistsEffectivelyEqual(
              currentGlobalNowPlaying.currentPlaylist, widget.songList) &&
          currentGlobalNowPlaying.currentIndexInPlaylist ==
              widget.initialIndex) {
    ...
    if (playThisSongAnuew) {
      MainTabsScreen.playNewSongInGlobalPlayer(
          widget.initialSong, widget.songList, widget.initialIndex,
          autoPlay: true);
    }
  ...
  void _setupLocalListeners() {
    _nowPlayingListenerCallback = () {
      ...
      if (mounted) {
        if (newNowPlaying != null) {
          if (_displayedSong.uniqueIdentifier !=
              newNowPlaying.song.uniqueIdentifier) {
    ...
    _shuffleModeSubscriptionLocal =
        globalAudioPlayer.shuffleModeEnabledStream.listen((_) {
      if (mounted) setState(() {});
    });
  }
  ...
  Future<void> _loadFavoriteStatusAndLyrics(Song songToCheck) async {
    ...
    if (username == null || username.isEmpty) {
      ...
      return; // Can't be a favorite if no one is logged in
    }
    ...
    final List<String> favoriteIdsList = _prefs!.getStringList(
  ...
  Future<void> _toggleFavorite() async {
    ...
    List<String> currentFavoriteDataStrings = _prefs!.getStringList(
            SharedPrefKeys.favoriteSongsDataListForUser(username)) ??
        [];
    List<String> currentFavoriteIds = _prefs!.getStringList(
            SharedPrefKeys.favoriteSongIdentifiersForUser(username)) ??
        [];
  ...
  void _controlPlayPause() {
    if (globalAudioPlayer.audioSource == null &&
        nowPlayingNotifier.value == null) return;
    ...
    if (globalAudioPlayer.shuffleModeEnabled && playlist.length > 1) {
      ...
      if (nextIndex >= playlist.length) {
        if (globalAudioPlayer.loopMode == LoopMode.all && playlist.isNotEmpty) {
          ...
          if (mounted)
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("End of playlist.")));
          return;
        }
      }
    }
    ...
    if (globalAudioPlayer.shuffleModeEnabled && playlist.length > 1) {
      ...
      if (availableIndices.isEmpty) {
        if (playlist.isNotEmpty)
          MainTabsScreen.playNewSongInGlobalPlayer(
              playlist[currentIndex], playlist, currentIndex,
              autoPlay: true);
        return;
      }
      availableIndices.shuffle(Random());
      ...
      if (prevIndex < 0) {
        if (globalAudioPlayer.loopMode == LoopMode.all && playlist.isNotEmpty) {
          ...
          if (mounted)
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Beginning of playlist.")));
          return;
        }
      }
    }
    MainTabsScreen.playNewSongInGlobalPlayer(
        playlist[prevIndex], playlist, prevIndex,
        autoPlay: true);
  }
  ...
  void dispose() {
    print(
        "SongDetailScreen: dispose for (displayed song) '${_displayedSong.title}'");
  ...
  Widget build(BuildContext context) {
    ...
    final bool isCurrentlyPlaying =
        nowPlayingNotifier.value?.isPlaying ?? globalAudioPlayer.playing;
    final bool canControlPlayback = globalAudioPlayer.audioSource != null ||
        nowPlayingNotifier.value != null;

    Widget coverWidget;
    if (songForDisplay.isLocal &&
        songForDisplay.mediaStoreId != null &&
        songForDisplay.mediaStoreId! > 0) {
      coverWidget = QueryArtworkWidget(
        id: songForDisplay.mediaStoreId!,
        type: ArtworkType.AUDIO,
        artworkFit: BoxFit.cover,
        artworkWidth: double.infinity,
        artworkHeight: screenHeight * 0.45,
        keepOldArtwork: true,
    ...
    } else if (songForDisplay.coverImagePath != null &&
        songForDisplay.coverImagePath!.isNotEmpty) {
      coverWidget = Image.asset(
        songForDisplay.coverImagePath!,
        fit: BoxFit.cover,
        width: double.infinity,
        height: screenHeight * 0.45,
        errorBuilder: (ctx, err, st) => Container(
      ...
      coverWidget = Container(
          ...
          color: colorScheme.surfaceVariant.withOpacity(0.7),
          child: Icon(Icons.album_rounded,
              size: 120, color: colorScheme.onSurfaceVariant.withOpacity(0.5)));
    }
                  ...
                  StreamBuilder<_PositionData>(
                    ...
                    builder: (context, snapshot) {
                      final positionData = snapshot.data ??
                          _PositionData(Duration.zero, Duration.zero,
                        ...
                        SliderTheme(
                          data: SliderTheme.of(context).copyWith(
                              ...
                              thumbShape: const RoundSliderThumbShape(
                                  enabledThumbRadius: 7.0, elevation: 1.0),
                              ...
                              overlayColor:
                                  colorScheme.primary.withAlpha(0x3D)),
                          child: Slider(
                            min: 0,
                            max: duration.inSeconds.toDouble().isFinite &&
                    ...
                    children: [
                      ...
                      IconButton(
                        icon: Icon(Icons.skip_previous_rounded,
                            ...
                            size: 40),
                        onPressed:
                            canControlPlayback ? _controlPlayPrevious : null,
                      ),
                      Container(
                      ...
                      StreamBuilder<LoopMode>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (loopMode == LoopMode.one) {
                            loopIcon = Icons.repeat_one_on_rounded;
                            tooltip = "Repeat One";
                          } else if (loopMode == LoopMode.all) {
                            loopIcon = Icons.repeat_on_rounded;
                            tooltip = "Repeat All";
                          } else {
