...
class MainTabsScreen extends StatefulWidget {
  ...
  static Future<void> playNewSongInGlobalPlayer(Song song, List<Song> playlist, int index, {bool autoPlay = true}) async {
    ...
    if (playlist.isEmpty || index < 0 || index >= playlist.length) {
      ...
      try { await globalAudioPlayer.stop(); } catch (e) { print("$methodTag: Error stopping player: $e"); }
    ...
    try {
      ...
      try { await globalAudioPlayer.stop(); } catch (_) {}
      if (playlist.isNotEmpty && index >= 0 && index < playlist.length) {
        nowPlayingNotifier.value = NowPlayingModel(
          song: playlist[index], audioPlayer: globalAudioPlayer, isPlaying: false,
          currentPlaylist: playlist, currentIndexInPlaylist: index,
        );
      } else {
        nowPlayingNotifier.value = null;
      }
...
class _MainTabsScreenState extends State<MainTabsScreen> {
  ...
  void _handleSongCompletion() {
    ...
    if (currentModel == null || currentModel.currentPlaylist.isEmpty) {
      ...
      if (currentModel != null && mounted) nowPlayingNotifier.value = currentModel.copyWith(isPlaying: false);
      return;
    }
    List<Song> playlist = currentModel.currentPlaylist;
    int currentIndex = currentModel.currentIndexInPlaylist;
    if (currentIndex < 0 || currentIndex >= playlist.length) {
      if (playlist.isNotEmpty) MainTabsScreen.playNewSongInGlobalPlayer(playlist[0], playlist, 0, autoPlay: true);
      else {
        globalAudioPlayer.stop().catchError((_) {});
        if (mounted) nowPlayingNotifier.value = currentModel.copyWith(isPlaying: false, currentPlaylist: [], currentIndexInPlaylist: 0);
      }
      return;
    }
    int nextIndex;
    if (globalAudioPlayer.shuffleModeEnabled && playlist.length > 1) {
      ...
      if (availableIndices.isEmpty) {
        if (globalAudioPlayer.loopMode == LoopMode.all && playlist.isNotEmpty) {
          ...
          globalAudioPlayer.pause(); globalAudioPlayer.seek(Duration.zero);
          if (mounted) nowPlayingNotifier.value = currentModel.copyWith(isPlaying: false);
          return;
        }
      } else {
        availableIndices.shuffle(Random());
  ...
  void _onItemTapped(int index) {
    ...
    if (_selectedIndex == index) {
      if (index == 0 && homeScreenKey.currentState != null) homeScreenKey.currentState!.scrollToTopAndRefresh();
      else if (index == 3 && localMusicScreenKey.currentState != null) localMusicScreenKey.currentState!.scrollToTopAndRefresh();
    }
    ...
    if (globalAudioPlayer.playing) {
      ...
      if (globalAudioPlayer.processingState == ProcessingState.completed) {
        ...
        MainTabsScreen.playNewSongInGlobalPlayer(
            currentModel.song, currentModel.currentPlaylist, currentModel.currentIndexInPlaylist, autoPlay: true
        );
      }
    }
  }
  ...
  Future<void> _showSortOptionsDialog(BuildContext context, String tabName, Function(String) onSortSelected) async {
    final String? selectedCriteria = await showDialog<String>(
      ...
      builder: (BuildContext context) {
        ...
        if (tabName == "My Music" || tabName == "Local Music" || tabName == "Favorites") {
          options.addAll([
        ...
        if (tabName == "Music Shop Category") { // فرض می‌کنیم نام تب برای لیست آهنگ‌های فروشگاه این است
          options.addAll([
            SimpleDialogOption(onPressed: () => Navigator.pop(context, 'price_asc'), child: const Text('Price (Low to High)')),
            SimpleDialogOption(onPressed: () => Navigator.pop(context, 'price_desc'), child: const Text('Price (High to Low)')),
            SimpleDialogOption(onPressed: () => Navigator.pop(context, 'rating_desc'), child: const Text('Rating (Highest First)')),
          ]);
        }
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedIndex == 0) { // My Music (HomeScreen)
      currentActions = [
    ...
    } else if (_selectedIndex == 3) { // Local Music (LocalMusicScreen)
      currentActions = [
        ...
        builder: (context, nowPlaying, child) {
          ...
          if (song.isLocal && song.mediaStoreId != null && song.mediaStoreId! > 0) {
            coverArtWidget = QueryArtworkWidget(
              id: song.mediaStoreId!, type: ArtworkType.AUDIO,
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      icon: Icon(
                        Icons.skip_next_rounded, size: 32,
                        color: canGoNext ? colorScheme.primary.withOpacity(0.9) : colorScheme.onSurface.withOpacity(0.35),
                      ),
                      onPressed: canGoNext ? _miniPlayerNext : null,
                      padding: EdgeInsets.zero, constraints: const BoxConstraints(), tooltip: "Next",
                    ),
                  ],
                ),
              ),
