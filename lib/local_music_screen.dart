...
class LocalMusicScreenState extends State<LocalMusicScreen> {
  ...
  String _currentSortCriteria = 'date_desc'; // پیش‌فرض: جدیدترین آهنگ‌ها از MediaStore
  ...
  Future<void> _requestPermissionAndLoadLocalSongs({bool forceRefresh = false}) async {
    ...
    setState(() {
      ...
      _errorMessage = '';
      if (forceRefresh) { // اگر رفرش اجباری است، لیست‌ها را پاک کن
    ...
    if (Platform.isAndroid) {
      var audioPermissionStatus = await Permission.audio.status; // یا Permission.storage بسته به SDK
    ...
    } else { // برای iOS یا پلتفرم‌های دیگر که نیازی به درخواست صریح مشابه ندارند
      permissionsGranted = true;
    }

    if (permissionsGranted) {
      try {
        print("LocalMusicScreen: Querying songs with sort: $_currentSortCriteria");
        ...
        if (deviceSongsRaw.isEmpty) {
          ...
          _localDeviceSongs = deviceSongsRaw
              .where((s) => (s.isMusic ?? false) && (s.duration ?? 0) > 30000) // حداقل 30 ثانیه
              .map((s) => Song(
            ...
            isLocal: true,
            mediaStoreId: s.id,
            dateAddedMediaStore: s.dateAdded, // timestamp از on_audio_query (ثانیه)
            // dateAdded اینجا null است
          ))
              .toList();

          if (_localDeviceSongs.isEmpty && deviceSongsRaw.isNotEmpty) {
            _errorMessage = "Audio files were found, but none met the criteria (e.g., music, duration > 30s).";
          } else if (_localDeviceSongs.isEmpty && deviceSongsRaw.isEmpty) { // این حالت با چک اولیه deviceSongsRaw.isEmpty پوشش داده می‌شود
            _errorMessage = "No music files found. Ensure music is in standard folders.";
          } else {
  ...
  void _applyFilter() {
    ...
    if (mounted) {
      setState(() { // باید setState اینجا باشد تا UI آپدیت شود
        if (query.isEmpty) {
  ...
  Future<void> refreshFavoritesOnReturn() async {
    if (!mounted) return;
    await _loadFavoriteSongIdentifiers();
    if (mounted) setState(() {}); // فقط برای رفرش UI اگر وضعیت آیکون قلب تغییر کرده
  }
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => SongDetailScreen(
          initialSong: song,
          songList: _filteredLocalDeviceSongs, // لیست فیلتر شده فعلی را به عنوان پلی‌لیست بفرست
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      ...
      bodyContent = Center(
          child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Icon(Icons.error_outline, size: 60, color: colorScheme.error.withOpacity(0.7)),
                  const SizedBox(height: 16),
                  Text(_errorMessage, textAlign: TextAlign.center, style: textTheme.titleMedium?.copyWith(color: colorScheme.onSurface.withOpacity(0.8))),
                  const SizedBox(height: 20),
                  ElevatedButton.icon(
                    icon: const Icon(Icons.refresh),
                    label: const Text("Try Again"),
                    onPressed: () => _requestPermissionAndLoadLocalSongs(forceRefresh: true),
                  )
                ],
              )
          )
      );
    } else if (_localDeviceSongs.isEmpty) { // این حالت با _errorMessage پوشش داده می‌شود اگر خطایی نباشد و لیستی خالی باشد
      bodyContent = Center(
          child: Padding(
              padding: const EdgeInsets.all(20.0),
              child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.music_off_outlined, size: 80, color: Colors.grey[700]),
                    const SizedBox(height: 24),
                    Text("No Local Music Found", style: textTheme.headlineSmall?.copyWith(color: colorScheme.onSurface.withOpacity(0.8))),
                    const SizedBox(height: 12),
                    Text("Scan again or check your device's music folders.", style: textTheme.bodyMedium?.copyWith(color: Colors.grey[600]), textAlign: TextAlign.center),
                    const SizedBox(height: 20),
                    ElevatedButton.icon(
                      icon: const Icon(Icons.refresh),
      ...
      bodyContent = Center(child: Text("No local music found for '${_searchController.text}'.", style: textTheme.titleMedium?.copyWith(color: colorScheme.onSurface.withOpacity(0.7))));
    } else {
      bodyContent = RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          padding: const EdgeInsets.only(bottom: 70.0, top: 8.0), // جا برای مینی پلیر
            ...
            return ListTile(
              ...
              leading: SizedBox(
                ...
                child: QueryArtworkWidget(
                  id: song.mediaStoreId ?? 0, // mediaStoreId برای آهنگ محلی الزامی است
                  type: ArtworkType.AUDIO,
                  artworkFit: BoxFit.cover,
                  artworkBorder: BorderRadius.circular(6.0),
                  artworkClipBehavior: Clip.antiAlias,
                  nullArtworkWidget: Container( // ویجت جایگزین اگر کاور وجود ندارد
                    width: 50, height: 50,
                  ...
                  errorBuilder: (_, __, ___) => Container( // ویجت جایگزین در صورت خطا در لود کاور
                    width: 50, height: 50,
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: _searchController.text.isNotEmpty ? IconButton(icon: Icon(Icons.clear_rounded, color: colorScheme.onSurface.withOpacity(0.6)), onPressed: () {
                _searchController.clear(); // listener _applyFilter را صدا می‌زند
              }) : null,
            ),
          ),
        ),
